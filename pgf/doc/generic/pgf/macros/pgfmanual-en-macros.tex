...
% $Header: /cvsroot/pgf/pgf/doc/generic/pgf/macros/pgfmanual-en-macros.tex,v 1.83 2016/02/23 23:56:48 tantau Exp $
...
\newenvironment{sysanimateattribute}[1]{
  \begin{pgfmanualentry}
    \pgfmanualentryheadline{%
      \pgfmanualpdflabel{#1}{}%
      \texttt{\string\pgfsys@animate\char`\{\declare{#1}\char`\}}%
    }
...
\newenvironment{animateattribute}[1]{
  \begin{pgfmanualentry}
    ...
    \index{#1@\protect\texttt{#1} animation attribute}%
    \index{Animation attributes!#1@\protect\texttt{#1}}%
    \pgfmanualbody
}
{
  \end{pgfmanualentry}
  ...
  \ifcode@execute%
    ...
    \setbox\codeexampleboxanim=\vbox{%
      ...
      \foreach \pgfmanualtime/\pgfmanualtimehow in\code@animation@list{%
        \setbox\codeexampleboxanim=\hbox{\colorbox{graphicbackground}{%
            \tikzset{make snapshot of=\pgfmanualtime}%
            \scalebox{.5}{\color{black}\ignorespaces%
