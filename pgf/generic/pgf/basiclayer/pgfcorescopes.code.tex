...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcorescopes.code.tex,v 1.48 2015/08/19 11:45:37 tantau Exp $
...
% 
% Prior to using \pgfbeginidscope, call \pgfuseid to install an
% id (and possibley \pgfusetype). The scope will then get this id and,
% likely, a graphic scope is installed. If \pgfuseid has not been
% called or if you try to call \pgfbeginidscope again for the same id,
...
\let\startpgfidscope\pgfbeginidscope
\let\stoppgfidscope\pgfendidscope


% Creates a new id, which can later be assigned to a scope, a path, or
% a text box.
% 
...
% Sets a key with an id
% 
% #1 = a name referencing an id
% #2 = a key
% #3 = a macro storing a value
%
% Description:
%
% Stores a value with a key for an id.

\def\pgfsetidkey#1#2#3{%
  \expandafter\global\expandafter\let\csname pgf@id@@#2@\pgfrefid{#1}\endcsname#3%
}


% Read some text from an id
%
% #1 = a macro
% #2 = a name referencing an id
% #3 = a key
%
% Description:
%
% Reads the value stored for an id and a key.

\def\pgfgetidkey#1#2#3{%
  \expandafter\ifx\csname pgf@scope@name@#2\endcsname\relax%
    \pgferror{Unknown id '#2'.}%
    \let#1\relax%
  \else%
    \expandafter\let\expandafter#1\csname pgf@id@@#3@\pgfrefid{#2}\endcsname%
  \fi%
}
...
% 
% #1 = a name previously created using \pgfnewid or \pgfaliasid. Sets
%      the type to the empty type as a side effect.
% 
% The next scope or path can be the target of an animation command or
% a hyperlink.

\def\pgfuseid#1{\pgfsys@use@id{\csname pgf@scope@name@#1\endcsname}\def\pgfcurrentid{#1}}
\let\pgf@scope@name@\pgfutil@empty


% Use a type
% 
% #1 = an id type; when starting with a dot, it is added to the current id
%     type.
% 
% The next scope or path can be the target of an animation command or
% a hyperlink.

\def\pgfusetype#1{\pgfsys@use@type{#1}}


% Expands to the current id in use
