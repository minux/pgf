...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/basiclayer/pgfcorescopes.code.tex,v 1.49 2015/08/19 22:47:27 tantau Exp $
...
% Description:
% 
% #1 will get the id that was last used by \pgfuseid for #2 or, if #2
% has not yet been used, the id that will be used next.

\def\pgfidrefprevuseornext#1#2{%
  \edef\pgf@temp{#2}\ifx\pgf@temp\pgfutil@empty\pgferror{Missing name reference for ``\string#1''}\fi%
  \pgf@lookup@id{#2}%
  \ifx\pgf@prev@id\pgfutil@empty%
    \let#1\pgf@next@id%
  \else%
    \let#1\pgf@prev@id%
  \fi%
...
% Description:
% 
% Depening on whether #1 has been forward-referenced, #2 or #3 will be executed.
