...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/tikzlibraryanimations.code.tex,v 1.14 2015/10/29 21:29:27 tantau Exp $
...
\def\tikz@anim@do#1{%
  {%
    \pgfkeys{/handlers/set/.code=\expandafter\expandafter\expandafter\tikz@animates@handler\expandafter\expandafter\expandafter{\pgfkeyscurrentpath}{##1}}%
...
\tikzanimationsset{
  all/set/.code=\tikzanimationsanimatesset{#1}
}
...
\def\tikz@anim@parser#1#2#3{%
  \tikz@anim@parser@scan@start{#1}{#2}<>#3<\pgf@stop%
}

\def\tikz@anim@parser@scan@start#1#2<{%
  \pgfutil@ifnextchar\pgf@stop{\pgfutil@gobble}{\begingroup\tikz@anim@parser@times{#1}{#2}}%
}
\def\tikz@anim@parser@scan@another#1#2<{%
  \pgfutil@ifnextchar\pgf@stop{\endgroup\pgfutil@gobble}{\tikz@anim@parser@times{#1}{#2}}%
}

\def\tikz@anim@parser@times#1#2#3>{%
  \pgfkeys{/tikz/animations/timing/.cd,#3}%
  \pgfutil@ifnextchar<{\tikz@anim@parser@scan@another{#1}{#2}}{\tikz@anim@parser@scan@value{#1}{#2}}
}
  
\def\tikz@anim@parser@scan@value#1#2#3<{%
  \pgfkeys@spdef\tikz@anim@value{#3}%
  \def\tikz@temp{\tikz@anim@parse@one{#1}{#2}}%
  \expandafter\tikz@temp\expandafter{\tikz@anim@value}%
  \endgroup%
  \tikz@anim@parser@scan@start{#1}{#2}<%
}

\def\tikz@anim@parse@one#1#2#3{%
  \def\tikz@temp{#3}%
