...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/tikzlibraryanimations.code.tex,v 1.20 2016/02/02 22:03:39 tantau Exp $
...
\tikzanimateset{
  ...
  value/.code=\tikz@animation@parser{#1},
  ...
  @/.code=\def\tikz@animation@time{#1},
  remember/.code=\pgfmathparse{\tikz@anim@t+(\tikz@anim@t@base)}\expandafter\global\expandafter\let\csname tikz@anim@rem@#1\endcsname\pgfmathresult,
  resume/.code={\pgfmathsetmacro\tikz@anim@t{\csname tikz@anim@rem@#1\endcsname-(\tikz@anim@t@base)}\tikz@animation@with{}do{}\pgf@stop\tikz@anim@time@scope},
}
...
\def\tikz@animation@with#1do#2\pgf@stop#3{%
  \begingroup%
    ...
    \let\tikz@anim@t@new@base\tikz@anim@t@base%
    \expandafter\expandafter\expandafter\def%
    \expandafter\expandafter\expandafter\tikz@anim@tl@options\expandafter\expandafter\expandafter{\expandafter\tikz@anim@tl@options\expandafter,\tikz@anim@head}%
    #3%
    \tikz@anim@make@entry%
    \let\tikz@anim@t@local\tikz@anim@t@saved%
    \ifx\tikz@anim@tail\pgfkeysnovalue@text\else%
      \begingroup%
        \let\tikz@anim@t@base\tikz@anim@t@new@base%
        \def\tikz@anim@t{0}%
        \let\tikz@anim@options\tikz@anim@options@record%
...
\def\tikz@anim@time@scope{%
  ...
  \else%
    % Tentatively execute the options to determine times...
    {%
      \let\tikz@anim@options\tikz@anim@options@time@only%
    ...
    \expandafter\expandafter\expandafter\def%
    \expandafter\expandafter\expandafter\tikz@anim@tl@options\expandafter\expandafter\expandafter{\expandafter\tikz@anim@tl@options\expandafter,\expandafter
      @\expandafter=\tikz@anim@t@new@base}%
  \fi%
}
...
\tikzanimationattributesset{
  .unknown/.code={
    ...
    \else%
      ...
      \expandafter\pgf@marshal\expandafter{\expandafter\tikz@anim@entry\expandafter{\tikz@anim@tl@options}}%
    \fi%
  }
}
...
\def\tikz@anim@entry#1{%
  ...
  \let\tikz@anim@options\tikz@anim@options@execute%
  \tikzanimateset{#1}%
...
\def\tikz@anim@options@time@only#1{
  % Is it a pgf key?
  \edef\tikz@anim@key{\pgfkeyscurrentname}%
  \pgfkeysifdefined{/tikz/animate/options/\tikz@anim@key/.@cmd}{%
...
\def\tikz@anim@options@execute#1{
  \edef\tikz@anim@key{\pgfkeyscurrentname}%
  \pgfkeys{/tikz/animate/options/\tikz@anim@key/.try={#1}}%
}

\def\tikz@anim@options@record#1{
  % Is it a pgf key?
  \edef\tikz@anim@key{\pgfkeyscurrentname}%
  \pgfkeysifdefined{/tikz/animate/options/\tikz@anim@key/.@cmd}{%
    \expandafter\expandafter\expandafter\def%
    \expandafter\expandafter\expandafter\tikz@anim@tl@options\expandafter\expandafter\expandafter{\expandafter\tikz@anim@tl@options\expandafter,\pgfkeyscurrentname={#1}}%
    \tikz@anim@make@entry%
  }{%
    \def\tikz@anim@unparsed@value{#1}%
    \expandafter\tikz@anim@time@test\tikz@anim@key\pgf@stop%
