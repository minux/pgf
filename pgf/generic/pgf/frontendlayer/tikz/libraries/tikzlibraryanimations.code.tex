...
\ProvidesFileRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/libraries/tikzlibraryanimations.code.tex,v 1.25 2016/03/06 14:03:23 tantau Exp $
...
\def\tikz@anim@dashpattern@parse#1{%
  ...
  \edef\tikz@anim@result{{\tikz@dashpattern}{\tikz@dashphase}}%
}
\def\tikz@anim@dashoffset@parse#1{%
  \pgfmathparse{#1}%
  \edef\tikz@anim@result{{\tikz@dashpattern}{\pgfmathresult}}%
}
\def\tikz@anim@dash@parse#1#2{%
  \pgfmathparse{#2}%
  \def\tikz@dashpattern{}%
  \expandafter\tikz@scandashon\pgfutil@gobble#1o\@nil%
  \edef\tikz@anim@result{{\tikz@dashpattern}{\pgfmathresult}}%
...
\tikzanimateset{
  ...
  repeat/.default=,
  add/.code=\tikz@anim@add{\pgfanimationset{add}},
  replace/.code=\tikz@anim@add{\pgfanimationset{replace}},
  ...
  paced/.code=\tikz@anim@add{\pgfanimationset{paced={#1}}},
  duration/.code=\tikz@anim@add{\pgfanimationset{paced={#1}}},
...
\tikzanimationdefineattribute{dash phase}{parser=dashoffset}
...
\tikzanimationdefineattribute{line width}{node type=.background.path}
