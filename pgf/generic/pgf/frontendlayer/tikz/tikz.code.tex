...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.161 2015/10/01 16:03:06 tantau Exp $
...
\tikzset{/tikz/animations/@default whom/.code=}
...
\def\tikz@auto@id{@auto}
\def\tikz@do@animation@option#1{%
  \ifx\tikz@id@name\pgfutil@empty%
    \let\tikz@id@name\tikz@auto@id%
  \fi%
  \edef\tikz@whom{\tikz@id@name}%
  {%
    \pgfkeys{/tikz/animations/@default whom/.expanded={#1}/{\tikz@whom}}%
    \tikz@animate@options%
  }%
  \let\tikz@animate@options=\pgfutil@empty%
}
\def\tikz@use@id@name{\pgfifidreferenced{\tikz@id@name}{}{\ifx\tikz@id@name\tikz@auto@id\let\tikz@id@name\pgfutil@empty\fi}\pgfuseid{\tikz@id@name}\pgfgaliasid{\tikz@id@name}{\tikz@id@name}}
...
\def\tikz@@scope@env[#1]{%
  ...
  \let\tikz@gs@objectkind\tikz@kind@token@scope%
  \tikzset{every scope/.try,#1}%
  \tikz@options%
  \tikz@do@animation@option{scope}%
