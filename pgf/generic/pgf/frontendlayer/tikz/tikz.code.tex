...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.154 2015/08/18 16:22:37 tantau Exp $
...
\def\tikz@@scope@env[#1]{%
  ...
  \ifx\tikz@id@name\pgfutil@empty\else\pgfnewid{\tikz@id@name}\pgfuseid{\tikz@id@name}\pgfsys@beginscope\fi%
  \begingroup%
    \let\tikz@id@name\pgfutil@empty%
...
\def\endtikz@scope@env{%
    \tikz@atend@scope%
  \endgroup%
  \ifx\tikz@id@name\pgfutil@empty\else\pgfsys@endscope\fi%
...
\def\tikz@finish{%
  ...
  % 
  \pgfnewid{g@path}%
  \pgfaliasid{@path}{g@path}%
  \ifx\tikz@animate@options\pgfutil@empty%
  \else%
    \pgfsys@beginscope%
      \tikz@animate@in@pathtrue%
      \tikz@animate@options%
      \tikz@animate@in@pathfalse%
  \fi%
  % 
  ...
  \ifx\tikz@id@name\pgfutil@empty%
    ...
    \fi%
  \else%
    \pgfuseid{@path}\pgfgaliasid{\tikz@id@name}{@path}%
  \fi%
  \let\tikz@id@name\pgfutil@empty%
  % 
      ...
      \hbox{{%
          \pgfinterruptpath%
            \pgfscope%
              ...
              \tikz@animate@options%
              \let\tikz@animate@options\pgfutil@empty%
              \ifx\tikz@id@name\pgfutil@empty\else\pgfgaliasid{\tikz@id@name}{g@node}\pgfuseid{g@node}\pgfsys@beginscope\fi%
              \begingroup%
                \let\tikz@id@name\pgfutil@empty%
                \setbox\tikz@figbox=\box\pgfutil@voidb@x%
                \setbox\tikz@figbox@bg=\box\pgfutil@voidb@x%
