...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.153 2015/08/18 08:49:35 tantau Exp $
...
\tikzset{
  name/.code={\edef\tikz@fig@name{\tikz@pp@name{#1}}\let\tikz@anim@group@name\tikz@fig@name},%
  name prefix/.initial=,%
  name suffix/.initial=%
}
...
\def\tikz@@scope@env[#1]{%
  ...
  \let\tikz@mode=\pgfutil@empty%
  \let\tikz@animate@options=\pgfutil@empty%
  \let\tikz@anim@group@name=\pgfutil@empty%
  ...
  \ifx\tikz@anim@group@name\pgfutil@empty\else\pgfanimationscope{\tikz@anim@group@name}\fi%
  \expandafter\tikz@atbegin@scope%
  \tikz@lib@scope@check%
}
\def\endtikz@scope@env{%
  \tikz@atend@scope%
  \ifx\tikz@anim@group@name\pgfutil@empty\else\endpgfanimationscope\fi%
...
\def\tikz@a@char{%
  \pgfutil@ifnextchar r{\tikz@arcA}{\tikz@animate@on@path}}%
\def\tikz@@e@char dge{%
...
% animate [options] { special path }
\def\tikz@animate@on@path nimate{\pgfutil@ifnextchar[\tikz@animate@on@path@opt{\tikz@animate@on@path@opt[]}}%
\def\tikz@animate@on@path@opt[#1]#2{\tikz@animate@path{#1}{#2}\tikz@scan@next@command}
\def\tikz@animate@path#1#2{
  \tikzerror{You need to say \string\usetikzlibrary{animations} to use the animate command}%
}
