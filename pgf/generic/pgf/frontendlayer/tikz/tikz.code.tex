...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/frontendlayer/tikz/tikz.code.tex,v 1.156 2015/08/19 11:45:37 tantau Exp $
...
% Animation options
\tikzset{every animate/.style=}
\tikzset{animations/.style={every animate/.append style={#1}}}
\tikzset{animate/.code=\expandafter\def\expandafter\tikz@animate@options\expandafter{\tikz@animate@options\tikz@animate{#1}}}
\def\tikz@animate@collect#1{\tikz@animate{#1}\pgfutil@ifnextchar;\pgfutil@gobble\relax}
\def\tikz@animate#1{\tikzerror{You need to say \string\usetikzlibrary{animations} to use the animate command}}
\let\tikz@animate@options\pgfutil@empty
...
\expandafter\let\csname tikz@protected@command\string\animate\endcsname\pgfutil@empty
...
\def\tikz@kind@token@scope{scope}
\def\tikz@kind@token@node{node}
\def\tikz@kind@token@path{path}
...
\def\tikz@@scope@env[#1]{%
  ...
  \ifx\tikz@id@name\pgfutil@empty\else\pgfnewid{@scope}\pgfuseid{@scope}\pgfgaliasid{\tikz@id@name}{@scope}\pgfsetidkey{@scope}{k}{\tikz@kind@token@scope}\fi%
  \pgfidscope%
    \begingroup%
      \let\tikz@id@name\pgfutil@empty%
...
\def\tikz@installcommands{%
  ...
  \let\animate=\tikz@animate@collect%
  %
  \def\draw{\path[draw]}
...
\def\tikz@finish{%
  ...
  \fi%
  %
  % Step: Name if necessary
  % 
  \ifx\tikz@id@name\pgfutil@empty%
    \ifx\tikz@animate@options\pgfutil@empty%
      \ifx\pgfcurrentid\tikz@@pathtext%
        \pgfuseid{}% no need for an id if it cannot be referenced
      \else%
        \pgfsetidkey{@path}{k}{\tikz@kind@token@path}
      \fi%
    \else%  
      \pgfsetidkey{@path}{k}{\tikz@kind@token@path}
    \fi%
  \else%
    \pgfgaliasid{\tikz@id@name}{@path}%
    \pgfsetidkey{@path}{k}{\tikz@kind@token@path}
  \fi%
  \let\tikz@id@name\pgfutil@empty%
  %
  ...
  % 
  \iftikz@mode@clip\else\pgfidscope\fi% open an animation scope here, unless clipping is done
  \tikz@animate@options%
  % 
  ...
  \iftikz@mode@fill%
    \iftikz@mode@shade%
      ...
      \pgfsys@get@type\tikz@type@temp%
      \pgfusetype{.path fill}%
      \pgfsys@fill%
      \pgfusetype{\tikz@type@temp}%
  ...
  %
  \iftikz@mode@clip\else\endpgfidscope\fi%
  % 
      ...
      \hbox\bgroup\bgroup%
          \pgfinterruptpath%
            \pgfscope%
              ...
              \tikz@animate@options%
              \let\tikz@animate@options\pgfutil@empty%
              \ifx\tikz@id@name\pgfutil@empty\else\pgfnewid{@matrix}\pgfuseid{@matrix}\pgfgaliasid{\tikz@id@name}{@matrix}\pgfsetidkey{@matrix}{k}{\tikz@kind@token@matrix}\fi%
              \pgfidscope%
              \begingroup%
                \let\tikz@id@name\pgfutil@empty%
                \setbox\tikz@figbox=\box\pgfutil@voidb@x%
            ...
            \pgfscope%
              ...
              \ifx\tikz@id@name\pgfutil@empty%
                \ifx\tikz@animate@options\pgfutil@empty%
                  \pgfuseid{}% no need for an id if it cannot be referenced
                \else%
                  \pgfsetidkey{@node}{k}{\tikz@kind@token@node}%
                \fi%
              \else%
                \pgfgaliasid{\tikz@id@name}{@node}%
                \pgfsetidkey{@node}{k}{\tikz@kind@token@node}%
              \fi%
              \pgfidscope%
              \tikz@animate@options%
