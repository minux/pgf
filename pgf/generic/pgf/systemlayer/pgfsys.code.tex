...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/systemlayer/pgfsys.code.tex,v 1.56 2015/08/19 22:47:27 tantau Exp $
...
% Sets the type to be used for the next graphic object. If it starts
% with a period, #1 is appended to the current type, otherwise #1
% replaces the current type.

\def\pgfsys@get@type#1{}
% Assigns the current type to the macro #1.

\def\pgfsys@new@id#1{\let#1\pgfutil@empty}
% Creates a new id and stores it in the macro #1.

\def\pgfsys@begin@idscope{\begingroup}
...
\def\pgfsys@end@idscope{\endgroup}
% Ends an id scope.
