...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/systemlayer/pgfsys.code.tex,v 1.58 2015/08/22 06:26:20 tantau Exp $
...
\def\pgfsys@end@idscope{
      \endgroup
    \csname pgf@sys@att@end@\pgfsys@id@refcurrent\endcsname%
...
\def\pgfsys@use@type#1{%
  \expandafter\let\expandafter\pgfsys@temp@type\csname pgf@sys@lookup@t@#1\endcsname%
  \if\pgfsys@temp@type\relax%
    \pgfsys@clean@type\pgfsys@temp@type{#1}%
    \expandafter\global\expandafter\let\csname pgf@sys@lookup@t@#1\endcsname\pgfsys@temp@type%
  \fi%
  \let\pgfsys@current@type\pgfsys@temp@type%
} 
\let\pgf@sys@lookup@t@\pgfutil@empty
\let\pgfsys@current@type\pgfutil@empty
...
\def\pgfsys@clean@type#1#2{\edef#1{#2}}
% Since types may be shown in the output file, it may be necessary to
% "clean" them like removing spaces or dots. This macro should store a
% clean version of #2 in #1.
...
% validated using \pgfsys@id@validate@type.
...
% gets created, #3 is added at the beginning and #4 is added at the end. 
...
\def\pgfsys@animateopacity{\pgfsys@animatefillopacity\pgfsys@animatestrokeopacity}
% Adds an animation of the opacity to the current scope. By default,
% animates both the fill and the stroke opacity, but a driver may
% choose to implement this directly.
% Type: scalar
...
\def\pgfsys@animatevisibility{\pgf@sys@fail{animations}}
% Adds an animation of the visiblity to the current scope.
...
\def\pgfsys@animation@origin#1#2{}
% Defines an origin for transformations like rotate and scale and
% skews. #1 and #2 must be given in pt.



% Values

\def\pgfsys@animation@text#1#2{}
% #1 should be some text. #2 is the key to which this scalar value is
