...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/systemlayer/pgfsys.code.tex,v 1.54 2015/08/18 23:56:36 tantau Exp $
...
\def\pgfsys@use@id#1#2{}
% The next graphics object (group or path) will get id and subid 
% assigned. The id and subid must previously have been created using 
% \pgfsys@new@id and each id-subid pair may be used only once.

\def\pgfsys@new@id#1{\global\let#1\pgfutil@empty}
% Creates a new id and stores it in the macro #1.

\def\pgfsys@new@id@type#1{\global\let#1\pgfutil@empty}
% Creates a new id type and stores it in the macro #1.

\def\pgfsys@begin@idscope{}
% Starts an "id scope" where the current value of the last
% \pgfsys@use@id is used. This may or may not create a graphic
% scope, depending on the driver. To ensure that a graphic scope is
% created, use \pgfsys@beginscope (which will also take the
% \pgfsys@use@id into account; this command was added so that for
% drivers that do not support ids, fewer graphic scopes are created in
% the output file).
