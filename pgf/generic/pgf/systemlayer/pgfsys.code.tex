...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/systemlayer/pgfsys.code.tex,v 1.52 2015/08/18 08:49:35 tantau Exp $
...
\def\pgfsys@begin@animation@scope{\begingroup}
% Starts a "animation scope" that can be referenced later on using an
% id. This id can be used to reference the scope as the target of an
% animation.
%
% An animation group may or may not constitute a graphic state
% group, add a normal if necessary.

\def\pgfsys@animationscope@id{}
% Will expand to the internal value of the current (innermost)
% animation scope. The value of #1 can be used with the
% \pgfsys@animation@whom command anywhere.

\def\pgfsys@end@animation@scope{\endgroup}
% Ends an animation scope with an id.




% All of the top-level animation commands start with
% \pgfsys@animate... They animate the given attribute. The
% configuration of the animation (like start times, etc.) is taken
...
%   \pgfsys@animation@scalar{0.5}{to}%
%   \pgfsys@animatefillopacity%
% }

\def\pgfsys@animatefillopacity{\pgf@sys@fail{animations}}
% Adds an animation of the fill opacity to the current scope.
% Type: scalar

\def\pgfsys@animatestrokeopacity{\pgf@sys@fail{animations}}
% Adds an animation of the stroke opacity to the current scope.
% Type: scalar

\def\pgfsys@animatelinewidth{\pgf@sys@fail{animations}}
% Adds an animation of the line width to the current scope.
% Type: dimension

\def\pgfsys@animatetranslate{\pgf@sys@fail{animations}}
% Adds an animation of the transformation to the current scope.
% Type: translate

\def\pgfsys@animatescale{\pgf@sys@fail{animations}}
% Adds an animation of the transformation to the current scope.
% Type: scale

\def\pgfsys@animaterotate{\pgf@sys@fail{animations}}
% Adds an animation of the transformation to the current scope.
% Type: rotate

\def\pgfsys@animateskewx{\pgf@sys@fail{animations}}
% Adds an animation of the transformation to the current scope.
% Type: skew

\def\pgfsys@animateskewy{\pgf@sys@fail{animations}}
% Adds an animation of the transformation to the current scope.
% Type: skew

\def\pgfsys@animatemotion{\pgf@sys@fail{animations}}
% Adds a movement along a path to the current scope.
% Type: motion

\def\pgfsys@animatestrokecolor{\pgf@sys@fail{animations}}
% Adds an animation of the stroke color to the current scope.
% Type: color

\def\pgfsys@animatefillcolor{\pgf@sys@fail{animations}}
% Adds an animation of the fill color to the current scope.
% Type: color

\def\pgfsys@animateviewbox{\pgf@sys@fail{animations}}
% Adds an animation of the view box to the current scope.
% Type: view box

\def\pgfsys@animatepath{\pgf@sys@fail{animations}}
% Adds an animation of the path to the next path.
% Type: path

\def\pgfsys@last@animation@id{}
% Will expand to the internal id of the last animation (globally). The
% value of this macro can be used as a target in \pgfsys@animation@event.

\def\pgfsys@next@animation@id{}
% Will expand to the next internal id that will be used for an
% animation (globally). The value of this macro can be used as a
% target in \pgfsys@animation@event. 


% Animation timing : Calls to the following macros are passed as 

\def\pgfsys@animation@dur#1{}
% #1 must be a clock value in seconds, defining the duration of an
% animation.
% Example: \pgfsys@animation@timing@dur{1.5} % one and a half second

\def\pgfsys@animation@min#1{}
% #1 must be a clock value, defining the minimum duration of an animation.

\def\pgfsys@animation@max#1{}
% #1 must be a clock value, defining the maximum duration of an animation.
...
\def\pgfsys@animation@keytime#1{}
% #1 must be a number. The semantics is as in SVS's
...
\def\pgfsys@animation@keyprogress#1{}
% #1 must be a numberd. The semantics is as in SVS's
...
\def\pgfsys@animation@keysplinecontrol#1#2#3#4{}
...
\def\pgfsys@animation@syncbegin#1#2#3{}
% #1 is the id of a sync base (obtained by
% \pgfsys@get@last@animation@id), #2 is an optional offset, #3 is the
% target.  

\def\pgfsys@animation@syncend#1#2#3{}
% #1 is the id of a sync base (obtained by
% \pgfsys@get@last@animation@id), #2 is an optional offset, #3 is the
% target. 
...
% \pgfsys@get@last@animation@id), #2 is an event name, and #3 is an
% optional offset, #4 is the target.  

\def\pgfsys@animation@repeat@event#1#2#3#4{}
% #1 is the optional id of another element (obtained by
% \pgfsys@get@last@animation@id), #2 is a repeat count, and
% #3 is an optional offset, #4 is the target. 
...
\def\pgfsys@animation@sum{}
% Corresponds to additive="sum" in SVG

\def\pgfsys@animation@replace{}
...
\def\pgfsys@animation@whom#1{}
% Sets the target of the animation. #1 must be a value previously
% retrieved using \pgfsys@get@animationscope@id.


% Attribute definitions

\def\pgfsys@animation@discrete{}
% Corresponds to calcMode="discrete" in SVG

\def\pgfsys@animation@linear{}
% Corresponds to calcMode="linear" in SVG

\def\pgfsys@animation@paced{}
...
% Applicable only to motion animations. #1 is a path along which the
...
\def\pgfsys@animation@scalar#1#2{}
% #1 should be a single value. #2 is the key to which this scalar
% value is added (must be "to", "from", "by" or "values")

\def\pgfsys@animation@dimension#1#2{}
% #1 must be a dimension. #2 is the key to which this value is added.

\def\pgfsys@animation@color@rgb#1#2#3#4{}
% #1,#2,#3 must be an rgb tuple, #4 is the key to which this is added

\def\pgfsys@animation@color@cmyk#1#2#3#4#5{}
% #1,#2,#3,#4 must be an cmyk tuple, #5 is the key to which this is added

\def\pgfsys@animation@color@cmy#1#2#3#4{}
% #1,#2,#3 must be an cmy tuple, #4 is the key to which this is added

\def\pgfsys@animation@color@gray#1#2{}
% #1 must be an rgb tuple, #2 is the key to which this is added

\def\pgfsys@animation@path#1#2{}
% #1 must consist of path-construction commands, #2 is the key to
% which this is added 

\def\pgfsys@animation@translate#1#2#3{}
% #1,#2 must be dimensions, #3 is the key to which this is added

\def\pgfsys@animation@scale#1#2#3{}
% #1,#2 must be dimensionless, #3 is the key to which this is added

\def\pgfsys@animation@rotate#1#2#3#4{}
% #1 must be dimensionless, #2,#3 describe the origin of the rotation
% and must be dimensions, #4 is the key to which this is added

\def\pgfsys@animation@viewbox#1#2#3#4#5{}
% #1,#2 describe the lower left, #3,#4 the upper right corner of the
% view box. #5 is the key to which this is added
