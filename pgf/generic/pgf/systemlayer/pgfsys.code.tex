...
\def\pgfsys@marker@declare#1#2#3{%
  {%
    ...
    \expandafter\xdef\csname pgf@sys@marker@shorten@#1\endcsname{#3}%
...
% Declares a new marker symbol whose code is in #2 and whose
% "protrusion" is in #3 (this is the distance by which a path should
% be shortened when the marker is added to it at the end or
% beginning). #1 should be a macro name, which will be set to a unique
% value by which the marker can be referenced later on. #2 should be
% code that, when executed, produces recordable code. 

\def\pgfsys@marker@get@shorten#1#2{%
  \expandafter\let\expandafter#2\csname pgf@sys@marker@shorten@#1\endcsname%
}
% #1 should be the id of a marker previously declared using the above
% command, #2 is a macro. It will be set to the shortening
% ("protrusion") value stored for #1.

\def\pgfsys@marker@at@begin@usedpath#1{\pgf@sys@fail{markers}}
% When called, the marker #1 should be at beginning of the paths
% that are used using commands like \pgfsys@stroke. The effect should
% be local to the current \TeX\ group. When the command is used twice,
% the last use wins.
%
% The semantics should follow the SVG-specification for markers.

\def\pgfsys@marker@at@end@usedpath#1{\pgf@sys@fail{markers}}
% Works like \pgfsys@marker@at@begin@usedpath, only for the end of a
% path. 

\def\pgfsys@marker@use#1{%
  \pgfsysprotocol@literal{\csname pgf@sys@marker@prot@#1\endcsname}%
}
% Draws the marker symbol not at the end of a path, put simply with
