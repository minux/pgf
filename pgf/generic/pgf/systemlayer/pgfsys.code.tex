...
\ProvidesPackageRCS[v\pgfversion] $Header: /cvsroot/pgf/pgf/generic/pgf/systemlayer/pgfsys.code.tex,v 1.71 2015/12/15 19:08:58 tantau Exp $
...
\def\pgfsys@attach@to@id#1#2#3#4#5{%
  \expandafter\def\expandafter\pgf@sys@tempsetup\expandafter{\csname pgf@sys@att@setup@\pgfsys@id@ref{#1}{#2}\endcsname}%
  ...
  \expandafter\ifx\pgf@sys@tempsetup\relax%
    \expandafter\let\pgf@sys@tempsetup\pgfutil@empty%
  \fi%
  ...
  \fi%
  \expandafter\let\expandafter\pgf@sys@tempsetup@cont\pgf@sys@tempsetup%
  ...
  \expandafter\expandafter\expandafter\gdef\expandafter\pgf@sys@tempsetup\expandafter{\pgf@sys@tempsetup@cont#5}%
...
% end. Furthermore, #5 is executed before the scope is created.
...
\def\pgfsys@animatestage{\pgf@sys@fail{animations}}
% Adds an animation of the staging: While the stage animation is not
% running, the object will be hidden, while it is running, it will be
% shown. 
% Type: none
...
\def\pgfsys@animatedashphase{\pgf@sys@fail{animations}}
% Adds an animation of the dash phase to the current scope.
% Type: dimension

\def\pgfsys@animatedashpattern{\pgf@sys@fail{animations}}
% Adds an animation of the dash array to the current scope.
% Type: dash array
...
\def\pgfsys@animatesyncbase{\pgf@sys@fail{animations}}
% Adds a sync base to the animation. A sync base does not animate
...
\def\pgfsys@animation@paced#1#2{}
% Specifies that the animation must be paced, starting at time #1 and
% ending at time #2. These fields may be empty; in this case the first
% and last times of entries will be used (altough the entries
% themselves will be ignored).

\def\pgfsys@animation@spline{}
% Specifies that the animation must be splined.
...
\def\pgfsys@animation@val{}
% Adds an entry with an empty ("nil") value. This is used by the stage
% animation.
...
% #1 must consist of path-construction commands
...
% #1 is a dash array with the syntax of the first argument of
% \pgfsys@setdash.
