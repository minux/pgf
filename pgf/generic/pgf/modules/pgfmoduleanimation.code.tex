...
%    
% In addition, before parsing #1, all occurrences of "08" and "09" get
% replaced by "8" and "9", respectively. This is to allow things like
% "01:08" for "68" since, otherwise, 08 is interpreted as an octal
% number. This may lead to errors when these are used in, say,
% strings. Do not rely one there errors, behaviour may change so that
% 08 is no longer parsed as an octal inside times.
...
\def\pgfparsetime#1{%
  \begingroup%
    ...
    \let\pgf@time@parsed\pgfutil@empty%
    \pgf@parse@time@strip@zeros#108\pgf@stop%
    \let\pgf@time@parsed@\pgfutil@empty%
    \expandafter\pgf@parse@time@strip@zeros@\pgf@time@parsed09\pgf@stop%
    \expandafter\pgfmathparse\expandafter{\pgf@time@parsed@}%
  \expandafter\endgroup%
  \expandafter\def\expandafter\pgftimeresult\expandafter{\pgfmathresult}  
}

\def\pgf@parse@time@strip@zeros#108#2\pgf@stop{%
  \def\pgf@temp{#2}%
  \ifx\pgf@temp\pgfutil@empty%
    \expandafter\def\expandafter\pgf@time@parsed\expandafter{\pgf@time@parsed#1}%
    \let\pgf@next\pgfutil@gobble%
  \else%
    \expandafter\def\expandafter\pgf@time@parsed\expandafter{\pgf@time@parsed#18}%
    \let\pgf@next\pgf@parse@time@strip@zeros%
  \fi%
  \pgf@next#2\pgf@stop%
}

\def\pgf@parse@time@strip@zeros@#109#2\pgf@stop{%
  \def\pgf@temp{#2}%
  \ifx\pgf@temp\pgfutil@empty%
    \expandafter\def\expandafter\pgf@time@parsed@\expandafter{\pgf@time@parsed@#1}%
    \let\pgf@next\pgfutil@gobble%
  \else%
    \expandafter\def\expandafter\pgf@time@parsed@\expandafter{\pgf@time@parsed@#19}%
    \let\pgf@next\pgf@parse@time@strip@zeros@%
  \fi%
  \pgf@next#2\pgf@stop%
}
...
% active. 
% 
% The style "only once" is a shorthand for "restart=false". 
%
% Example:
% 
...
% \pgfanimateattribute{fill opacity}{begin on={click, of=button}, duration=1s, to=0}
%
...
\def\pgfanim@make@event#1#2{%
  ...
  \ifx\pgfanim@event@key\pgfutil@empty\else%
    \pgfsys@animation@accesskey{\pgfanim@event@key}{\pgfanim@event@delay}{#2}%
  \else\ifx\pgfanim@event@repeat\pgfutil@empty\else%
