...
\def\pgfarrow@marker@setup@end{%
  \ifx\pgf@arrowpath\pgfutil@empty\else%
    \ifx\pgf@end@tip@sequence\pgfutil@empty\else%
      % Ok, need to draw something
      \ifx\pgf@do@draw@end\pgf@do@draw@straightend%
        {%
          \pgftransformreset%
          \pgf@arrow@prep@marker@arrow\pgf@end@tip@sequence\pgf@arrow@tip@total@length\pgf@temp%
        \expandafter}%
        \expandafter\pgfsys@marker@at@end@usedpath\expandafter{\pgf@temp}%
        {%
          \pgftransformreset%
          \pgftransformarrow{\pgfqpoint{\pgf@xc}{\pgf@yc}}{\pgfqpoint{\pgf@xb}{\pgf@yb}}%
          \pgf@arrow@rigid@arrow\pgf@end@tip@sequence\pgf@arrow@tip@total@length%
        }%
      \else%
        \pgferror{Cannot draw bend and flex arrow tips as markers}%
      \fi%
    \fi%    
  \fi%
}

\def\pgfarrow@marker@setup@begin{%
  \ifx\pgf@arrowpath\pgfutil@empty\else%
    \ifx\pgf@start@tip@sequence\pgfutil@empty\else%
      % Ok, need to draw something
      \ifx\pgf@do@draw@start\pgf@do@draw@straightstart%
        {%
          \pgftransformreset%
          \pgf@arrow@prep@marker@arrow\pgf@start@tip@sequence\pgf@arrow@tip@total@length\pgf@temp%
        \expandafter}%
        \expandafter\pgfsys@marker@at@begin@usedpath\expandafter{\pgf@temp}%
        {%
          \pgftransformreset%
          \pgftransformarrow{\pgfqpoint{\pgf@xc}{\pgf@yc}}{\pgfqpoint{\pgf@xb}{\pgf@yb}}%
          \pgf@arrow@rigid@arrow\pgf@start@tip@sequence\pgf@arrow@tip@total@length%
        }%
      \else%
        \pgferror{Cannot draw bend and flex arrow tips as markers}%
      \fi%
...
\def\pgf@arrow@prep@marker@arrow#1#2#3{%
  % #1 = tip sequence
  % #2 = pgf@arrow@tip@total@length
  % #3 = macro for storing marker code
  %
  % First, compute id's:
  {%
    \let\pgf@arrow@handle\pgfarrow@marker@id@test%
    \let\pgf@arrow@handle@dot\relax%?
    \global\let\pgfarrow@marker@id@list\pgfutil@empty%
    #1%
  }%
  \expandafter\let\expandafter#3\csname pgfsys@marker@@cache@\pgfarrow@marker@id@list\endcsname%
  \ifx#3\relax% ok, have to create it!
    {%
      \let\pgf@arrow@handle\pgf@arrow@drawer@marker
      \let\pgf@arrow@handle@dot\relax%
      \pgfsys@marker@declare#3{%
        \pgf@xb=#2%
        \pgftransformxshift{-\pgf@xb}%
        \pgftransformxshift{\pgf@path@shortening@distance}%
        #1%
      }%
    }%
  \fi%
...
\def\pgfarrow@marker@id@test#1#2{%
  \pgfarrows@getid{#1}{#2}%
  \expandafter\expandafter\expandafter\pgf@arrow@drawer@shift\csname pgf@ar@ends@\pgf@arrow@id\endcsname%
  \xdef\pgfarrow@marker@id@list{\pgfarrow@marker@id@list:\pgf@arrow@id\ifpgfarrowswap s\else n\fi\pgfarrows@slant w\the\pgf@xc}%
}

\def\pgf@arrow@drawer@marker#1#2{%
  % Prepare:
  {%
    \pgfarrows@getid{#1}{#2}%
    % Do shift:
...
\newif\ifpgfarrow@marker
% When set to true and when a driver supports markers, pgf will use
% markers (see the SVG spec) for arrow heads rather than "explicit"
% code. This saves space and works well with animations.

\pgfkeys{/pgf/draw arrows using markers/.is if=pgfarrow@marker}
