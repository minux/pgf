...
\ProvidesFileRCS $Header: /cvsroot/pgf/pgf/generic/pgf/modules/pgfmoduleanimations.code.tex,v 1.9 2015/08/20 12:20:19 tantau Exp $
...
\pgfkeys{/pgf/@animation attributes/.cd,
  ...
  origin/.code=\pgf@animate@attr{origin}{point},
...
\pgfanimationset{
  duration/.code=\pgfparsetime{#1}\pgfsysanimkeyduration{\pgftimeresult},
...
\pgfanimationset{
  min duration/.code=\pgfparsetime{#1}\pgfsysanimkeyminduration{\pgftimeresult},
  max duration/.code=\pgfparsetime{#1}\pgfsysanimkeymaxduration{\pgftimeresult}
}
...
\pgfanimationset{
  add/.is choice,
  add/.default=true,
  add/true/.code=\pgfsysanimkeysum,
  add/false/.code=\pgfsysanimkeyreplace
}
...
\pgfkeys{/pgf/animation/events/.cd,
  ...
  of/.code={%
    \pgf@anim@decomp{#1}%
    \pgfidrefprevuseornext\pgf@anim@id\pgf@anim@id@name%
  },
...
\pgfanimationset{
  ...
  interpolation/discrete/.code=\pgfsysanimkeydiscrete,
  interpolation/linear/.code=\pgfsysanimkeylinear,
  interpolation/paced/.code=\pgfsysanimkeypaced,
  interpolation/spline/.code=\pgfsysanimkeyspline
}
...
\pgfanimationset{
  key time/.code=\pgfmathparse{#1}\pgfsysanimkeykeytime{\pgfmathresult},
  key progress/.code=\pgfmathparse{#1}\pgfsysanimkeykeyprogress{\pgfmathresult},
  key spline control/.code n args={4}{%
    ...
    \pgfsysanimkeykeysplinecontrol{\pgfanim@cont@a}{\pgfanim@cont@b}{\pgfanim@cont@c}{\pgfanim@cont@d}%
  }
}
...
% #1 = a pgf point
%
% Specifies the origin of transformations like a rotate or a scale. 
%
% Example:
% 
% \pgfanimateattribute{rotate}{duration=1s, from=0, to=90, origin=\pgfpoint{1cm}{1cm}}

\pgfanimationset{
  origin/.code= \pgf@process{#1}\pgfsysanimkeyorigin{\the\pgf@x}{\the\pgf@y}%
}
...
\def\pgfanim@parse@type@scalar#1#2{%
  \pgfmathparse{#1}%
  \pgfsysanimkeyscalar{\pgfmathresult}{#2}%
}
...
\def\pgfanim@parse@type@mapped#1#2{%
  {%
    ...
    \fi%
  \expandafter}%
  \expandafter\pgfsysanimkeytext\expandafter{\pgf@temp}{#2}%
}
...
\def\pgfanim@parse@type@path#1#2{%
  ...
  \pgfsysanimkeypath{\pgfanim@the@path}{#2}%
}
...
\def\pgfanim@parse@type@softpath#1#2{%
  \pgfsysanimkeypath{#1}{#2}%
}
...
\def\pgfanim@parse@type@point#1#2{%
  \pgf@process{#1}%
  \pgfsysanimkeytranslate{\pgf@x}{\pgf@y}{#2}%
}
...
\def\pgfanim@parse@type@viewbox#1#2{%
  {%
    ...
    \xdef\pgf@anim@caller{\noexpand\pgfsysanimkeyviewbox{\the\pgf@x}{\the\pgf@y}{\the\pgf@xa}{\the\pgf@ya}{#2}}%
  }% 
  \pgf@anim@caller%
}
